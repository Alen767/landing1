/*
Cache: jquery-lazy, thickbox, dt-main, dt-language, dt-main-op-jquery, dt-plugins, waypoints, vc_carousel_js, vc_transition_bootstrap_js, qfe_composer_front_js, wc-cart-fragments, jquery-cookie, wc-checkout, wc-address-i18n, wc-country-select, bitcommerce, jquery-blockui, jquery-form
*/
...
/* wc-address-i18n: (https://59f041758befa.t74.qifeiye.com/qfy-content/plugins/bitcommerce/assets/js/frontend/address-i18n.min.js) */
jQuery(function(a){var b=wc_address_i18n_params.locale.replace(/&quot;/g,'"'),c=a.parseJSON(b),d=' <abbr class="required" >*</abbr>';a("body").bind("country_to_state_changing",function(b,e,f){var g=f;if("undefined"!=typeof c[e])var h=c[e];else var h=c["default"];var i=a.parseJSON(wc_address_i18n_params.locale_fields);a.each(i,function(a,b){var e=g.find(b);h[a]?(h[a].label&&e.find("label").html(h[a].label),h[a].placeholder&&e.find("input").attr("placeholder",h[a].placeholder),e.find("label abbr").remove(),"undefined"==typeof h[a].required&&1==c["default"][a].required?e.find("label").append(d):1==h[a].required&&e.find("label").append(d),"state"!==a&&(1==h[a].hidden?e.hide().find("input").val(""):e.show())):c["default"][a]&&(1==c["default"][a].required&&0==e.find("label abbr").size()&&e.find("label").append(d),"state"!==a&&("undefined"==typeof c["default"][a].hidden||0==c["default"][a].hidden?e.show():1==c["default"][a].hidden&&e.hide().find("input").val("")))});var j=g.find("#billing_postcode_field, #shipping_postcode_field"),k=g.find("#billing_city_field, #shipping_city_field"),l=g.find("#billing_state_field, #shipping_state_field");j.attr("data-o_class")||(j.attr("data-o_class",j.attr("class")),k.attr("data-o_class",k.attr("class")),l.attr("data-o_class",l.attr("class")));
	if( jQuery(".bitcommerce-billing-fields.CNSTYLE").length==0){
		h.postcode_before_city?(j.add(k).add(l).removeClass("form-row-first form-row-last").addClass("form-row-wide"),j.insertBefore(k)):(j.attr("class",j.attr("data-o_class")),k.attr("class",k.attr("data-o_class")),l.attr("class",l.attr("data-o_class")),j.insertAfter(l))
	}
})});;

/* wc-checkout: (https://59f041758befa.t74.qifeiye.com/qfy-content/plugins/bitcommerce/assets/js/frontend/checkout.min.js) */
jQuery(function($) {
	...
	function update_checkout() {
		...
		var data = {
			action: 			'bitcommerce_update_order_review',
		...
		xhr = $.ajax({
			...
			success: 	function( response ) {
				if ( response ) {
					...
					if(parent.qfy_product_style_set) parent.qfy_product_style_set();
					if( jQuery('.shipping_method [value="ecpay_shipping"],.shipping_method[value="ecpay_shipping"]').length>0){
						 if(jQuery(".csv_map_html").length>0){
	                         var maphtm=  jQuery(".csv_map_html").prop("outerHTML");
	                         jQuery(".csv_map_html").remove();
	                         maphtm = maphtm.replace("<cform","<form").replace("</cform>","</form>");
							 jQuery("form[name='checkout']").after(maphtm);
						 }
	                     if(jQuery("#shipping_method_0_ecpay_shipping:checked").length>0){
	                    	 	jQuery("#账单字段960_field input").val("");
								jQuery("#账单字段960_field").hide();
                         }else{
                        	 	jQuery("#账单字段960_field").show();
                         }

                     }
					if(typeof shipping_option_change=="function"){
						shipping_option_change("init");
					}
	...
	.on( 'input change', '.address-field input.input-text, .update_totals_on_change input.input-text', function() {
		if ( dirtyInput ) {
			clearTimeout( updateTimer );
			updateTimer = setTimeout( input_changed, '1500' );
		}
	})

	.on( 'input change', '.address-field select,select.address-field', function() {
		dirtyInput = this;
		input_changed();
	})
	...
	.on( 'blur input change', '.input-text, select', function() {
		...
		if ( $parent.is( '.validate-required' ) ) {
			if ( $this.val() == '' ) {
				$parent.removeClass( 'bitcommerce-validated' ).addClass( 'bitcommerce-invalid bitcommerce-invalid-required-field' );
				validated = false;
			}
		}
		...
		if ( $parent.is( '.validate-email' ) ) {
			if ( $this.val() ) {
				...
				if ( ! pattern.test( $this.val()  ) ) {
					$parent.removeClass( 'bitcommerce-validated' ).addClass( 'bitcommerce-invalid bitcommerce-invalid-email' );
					validated = false;
				}
			}
		}
		...
		if ( validated ) {
			$parent.removeClass( 'bitcommerce-invalid bitcommerce-invalid-required-field' ).addClass( 'bitcommerce-validated' );
		}
	} )
	...
	.submit( function() {
		...
		if ( $form.triggerHandler('checkout_place_order') !== false && $form.triggerHandler('checkout_place_order_' + $('#order_review input[name=payment_method]:checked').val() ) !== false ) {
			...
			$form.find("#place_order").attr("disabled","disabled");
			...
			$.ajax({
				...
				success: 	function( code ) {
					...
					try {
						...
						// Remove old errors
						$('.bitcommerce-error, .bitcommerce-message').remove();
	...
	$('form.checkout_coupon').submit( function() {
		...
		var data = {
			action: 			'bitcommerce_apply_coupon',
		...
		$.ajax({
			...
			success: 	function( code ) {
				...
				if ( code ) {
					//$form.before( code );
					//$form.slideUp();
					var text =  jQuery.trim(jQuery(code).text());
					 qfy_popinfo_fun('<div style="text-align:center;" ><div style="padding:20px;">'+text+'</div></div>',3);
					$('body').trigger('update_checkout');
				}
			},
			dataType: 	"html"
...
function ecp_map_html(e){
        e.preventDefault();
	 	e.stopPropagation();
	 	var billing_first_name = jQuery("#billing_first_name").val();
	 	var billing_phone = jQuery("#billing_phone").val();
	 	var field  = jQuery("#账单字段412").val();
	 	var order_comments = jQuery("#order_comments").val();
	 	 var v = jQuery('#ECPayForm #ServerReplyURL').val();

	    if(v.indexOf("?")>-1){
	    	if(jQuery("#billing_field_206").length>0) {
				var billing_last_name = jQuery("#billing_last_name").val();
				jQuery('#ECPayForm #ServerReplyURL').val(v + "&billing_last_name=" + encodeURIComponent(billing_last_name) + "&billing_phone=" + encodeURIComponent(billing_phone) + "&billing_field_34=" + encodeURIComponent(jQuery("#billing_field_34").val()) + "&billing_field_206=" + encodeURIComponent(jQuery("#billing_field_206").val()) + "&order_comments=" + encodeURIComponent(order_comments));
			}else if(location.host=="www.alexshop.tw"){
				var billing_country = jQuery("#billing_country").val();
				jQuery('#ECPayForm #ServerReplyURL').val(v + "&billing_country=" + encodeURIComponent(billing_country)  + "&billing_first_name=" + encodeURIComponent(jQuery("#billing_first_name").val()) + "&billing_phone=" + encodeURIComponent(jQuery("#billing_phone").val()) + "&order_comments=" + encodeURIComponent(order_comments));
	    	}else{
	 			jQuery('#ECPayForm #ServerReplyURL').val(v+"&billing_first_name="+encodeURIComponent(billing_first_name)+"&billing_phone="+encodeURIComponent(billing_phone)+"&账单字段412="+encodeURIComponent(field)+"&order_comments="+encodeURIComponent(order_comments) );
			}
	    }else{
	    	jQuery('#ECPayForm #ServerReplyURL').val(v+"?billing_first_name="+encodeURIComponent(billing_first_name)+"&billing_phone="+encodeURIComponent(billing_phone)+"&账单字段412="+encodeURIComponent(field)+"&order_comments="+encodeURIComponent(order_comments));
	    }
	 	 jQuery('#ECPayForm #__paymentButton').click();
 }
;
